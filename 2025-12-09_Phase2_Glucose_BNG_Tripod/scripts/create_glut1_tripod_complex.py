#!/usr/bin/env python3
"""
Create GLUT1-Tripod complex for Membrane Builder
Aligns Phase 2 coordinates to new glycosylated GLUT1 structure
"""
...
def kabsch_align(P, Q):
    ...
    P: reference (target) coordinates
    Q: mobile coordinates to align
    Returns: rotation matrix R, translation t
    """
    # Center
...
def write_pdb(atoms, filename, title="Structure"):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(f"REMARK   Generated by create_glut1_tripod_complex.py\n")
...
def main():
    print("=" * 80)
    print("GLUT1-Tripod Complex Generator")
    print("=" * 80)
    
    # File paths
    phase2_glut1_file = "phase2_glut1_coords.pdb"
    phase2_tripod_file = "phase2_tripod_coords.pdb"
    new_glut1_file = "/home/pjho3/다운로드/charmm-gui-6750265216membranebuilder/step1_pdbreader.pdb"
    charmm_tripod_file = "charmm_gui_tripod.pdb"
    
    print("\n📁 Input files:")
    print(f"  Phase 2 GLUT1: {phase2_glut1_file}")
    print(f"  Phase 2 Tripod: {phase2_tripod_file}")
    print(f"  New GLUT1 (glycosylated): {new_glut1_file}")
    print(f"  CHARMM-GUI Tripod: {charmm_tripod_file}")
    
    # Step 1: Read Phase 2 structures
    print("\n📖 Step 1: Reading Phase 2 structures...")
    ...
    # Step 2: Read new GLUT1 structure
    print("\n📖 Step 2: Reading new glycosylated GLUT1...")
    new_glut1_ca = read_pdb_atoms(new_glut1_file, select_type="CA")
    new_glut1_all = read_pdb_atoms(new_glut1_file)
    
    print(f"  New GLUT1 CA atoms: {len(new_glut1_ca)}")
    print(f"  New GLUT1 total atoms: {len(new_glut1_all)}")
    
    # Step 3: Align GLUT1 structures
    print("\n🔄 Step 3: Aligning GLUT1 structures (Kabsch)...")
    
    # Extract coordinates
    P = np.array([atom['coord'] for atom in new_glut1_ca])  # Target (new)
    ...
    # Step 4: Transform Phase 2 Tripod to new coordinate system
    print("\n🎯 Step 4: Transforming Tripod coordinates...")
    ...
    # Step 5: Change residue name from UNL to TRP
    print("\n✏️  Step 5: Updating Tripod residue name (UNL → TRP)...")
    for atom in transformed_tripod:
        line = atom['line']
        atom['line'] = line[:17] + 'TRP' + line[20:]
    
    # Step 6: Combine GLUT1 + Tripod
    print("\n🔗 Step 6: Creating complex...")
    complex_atoms = new_glut1_all + transformed_tripod
    
    print(f"  Total atoms in complex: {len(complex_atoms)}")
    print(f"    GLUT1: {len(new_glut1_all)}")
    ...
    # Complex PDB
    write_pdb(complex_atoms, "glut1_tripod_complex.pdb", 
              "GLUT1 (glycosylated) + Tripod complex")
    print("  ✅ glut1_tripod_complex.pdb")
    
    # Tripod only (aligned)
    write_pdb(transformed_tripod, "tripod_aligned.pdb",
              "Tripod aligned to new GLUT1 structure")
    print("  ✅ tripod_aligned.pdb")
    
    # GLUT1 only
    write_pdb(new_glut1_all, "glut1_glycosylated.pdb",
              "Glycosylated GLUT1")
    print("  ✅ glut1_glycosylated.pdb")
    ...
    print("\n📋 Next steps:")
    print("  1. Visual check: vmd glut1_tripod_complex.pdb")
    print("  2. Upload to CHARMM-GUI Membrane Builder")
    print("  3. Add membrane and run MD simulation")
