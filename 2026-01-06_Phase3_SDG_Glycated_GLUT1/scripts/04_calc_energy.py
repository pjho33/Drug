...
def calculate_binding_energy(trajectory_file, ligand_file, output_log):
    ...
    for i, ch in enumerate(chains):
        n_atoms = sum(1 for _ in ch.atoms())
        print(f"      Chain {i}: {ch.id} with {n_atoms} atoms")
    
    # Receptor Only (Chain A = 첫 번째 chain)
    ...
    # 리간드 chain (마지막) 삭제
    lig_chain = [ch for ch in chains_rec if ch.id == 'B' or ch.id == chains_rec[-1].id]
    ...
    # 단백질 chain (첫 번째) 삭제
    rec_chain = [ch for ch in chains_lig if ch.id == 'A' or ch.id == chains_lig[0].id]
    if rec_chain: modeller_lig.delete(rec_chain)
