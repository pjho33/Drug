...
OUTPUT_PNG = os.path.join(RESULTS_DIR, "tripod_10ns_plots.png")
...
def main():
    ...
    fig.suptitle('Tripod MD Simulation Analysis (10 ns)', fontsize=16, fontweight='bold')
    ...
    ax1.plot(arm_data['time_ns'], arm_data['arm3_A'], 'g-', label='Arm 3', linewidth=1.5, alpha=0.8)
    ...
    # 2. Max arm length
    ax2 = axes[0, 1]
    ax2.plot(arm_data['time_ns'], arm_data['max_arm_A'], 'purple', linewidth=1.5, alpha=0.8)
    ax2.axhline(y=arm_data['max_arm_A'].mean(), color='darkviolet', linestyle='--', 
                linewidth=2, label=f'Mean: {arm_data["max_arm_A"].mean():.1f} Å')
    ax2.axhline(y=40, color='orange', linestyle='--', linewidth=1, alpha=0.5, label='40 Å')
    ax2.axhline(y=50, color='red', linestyle='--', linewidth=1, alpha=0.5, label='50 Å')
    ...
    ax3 = axes[0, 2]
    ax3.plot(arm_data['time_ns'], arm_data['Rg_A'], 'orange', linewidth=1.5, alpha=0.8)
    ...
    ax5 = axes[1, 1]
    ax5.hist(arm_data['arm1_A'], bins=20, alpha=0.6, label='Arm 1', color='blue', edgecolor='black')
    ax5.hist(arm_data['arm2_A'], bins=20, alpha=0.6, label='Arm 2', color='red', edgecolor='black')
    ax5.hist(arm_data['arm3_A'], bins=20, alpha=0.6, label='Arm 3', color='green', edgecolor='black')
    ...
    p40 = (arm_data['max_arm_A'] >= 40).mean() * 100
    p50 = (arm_data['max_arm_A'] >= 50).mean() * 100
