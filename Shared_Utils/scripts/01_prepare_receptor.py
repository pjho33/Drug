...
class ChainSelect(PDB.Select):
    ...
    def accept_chain(self, chain):
        # Trimer 모드: 모든 체인 유지 (Tripod 리간드용)
        if self.keep_all:
            return True
        # Monomer 모드: 첫 번째 체인만 선택
        model = chain.get_parent()
        first_chain_id = list(model.get_chains())[0].id
        return chain.id == first_chain_id

def prepare_receptor(input_pdb, output_pdb, keep_all_chains=False):
    print(f"🛠️ [Step 1] Preparing Receptor (High-End Mode): {input_pdb}")
    
    # ----------------------------------------------------
    # 1. Biopython으로 체인 추출 (Extraction)
    # ----------------------------------------------------
    parser = PDB.PDBParser(QUIET=True)
    structure = parser.get_structure("protein", input_pdb)
    
    # Trimer 감지: 파일명에 'trimer'가 있거나 체인이 3개면 전체 유지
    chain_count = len(list(structure.get_chains()))
    is_trimer = 'trimer' in input_pdb.lower() or chain_count == 3
    
    if is_trimer or keep_all_chains:
        print(f"   🔺 Trimer detected ({chain_count} chains) - keeping all chains...")
        selector = ChainSelect(keep_all=True)
    else:
        print("   ✂️ Extracting Chain A using Biopython...")
        selector = ChainSelect(keep_all=False)
